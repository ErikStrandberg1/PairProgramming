...
void Actor::Update(const UpdateContext& updateContext)
{
	...
	const Tga::Vector2f steeringForce = myController->Update(updateContext, { myPosition, myVel });
	...
	if (myController->ShouldFaceVelocity() && myVel.LengthSqr() > STOP_SPEED)
	{
		myDirection = myVel.GetNormalized();
		myRotation = std::atan2(myDirection.y, myDirection.x);
	}
