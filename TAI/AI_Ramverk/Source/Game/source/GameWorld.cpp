...
void GameWorld::Init()
{
	...
	for (int i = 0; i < enemyCount; ++i)
	{
		...
		Vector2f startPos = {.3f + (float)i / 10, .3f + (float)i / 10};

		float speed = (float)(rand() % 10) / 100.f;
		enemy->Init(
			"../data/sprites/killerRobo1.png",
			speed,
			myControllerFactory.CreateController(AI::eControllerType::eEnemy, AI::eSteeringType::eSeek),
			startPos
		);
	...
	{
		auto enemy = std::make_unique<Actor>();
		Vector2f startPos = {(float)(rand() % 10) / 10.f, (float)(rand() % 10) / 10.f};

		enemy->Init(
			"../data/sprites/killerRobo1.png",
			0.10f,
			myControllerFactory.CreateController(AI::eControllerType::eEnemy, AI::eSteeringType::eSeparation),
			startPos
		);
