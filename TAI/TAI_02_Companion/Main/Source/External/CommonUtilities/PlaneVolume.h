...
namespace CommonUtilities
{
	...
	public:
		...
		std::vector<Plane<T>> GetPlanes() { return myPlanes; };
		// Returns whether a point is inside the PlaneVolume: it is inside when the point is
		//on the
		// plane or on the side the normal is pointing away from for all the planes in the
		//PlaneVolume.
	...
	template <typename T>
	PlaneVolume<T>::PlaneVolume(const std::vector<Plane<T>>& aPlaneList) : myPlanes(aPlaneList)
	{
	}
